    ...
    private void LoadActiveAdvertisements()
    {
        try
        {
            DataTable dt = DBHelper.ExecuteQuery(
                @"SELECT AdvertisementID, Title, Description, ImagePath, CreatedDate
              FROM Advertisements
              WHERE Status = 'Active'
            ...
            {
                int id = Convert.ToInt32(row["AdvertisementID"]);
                ...
                ads.Add(new
                {
                    ...
                    imagePath = string.IsNullOrEmpty(imagePath) ? "" : ResolveUrl(imagePath)
                });

                if (latestId == 0) { latestId = id; }
            }

            string json = new JavaScriptSerializer().Serialize(new { ads, latestId });
            Page.ClientScript.RegisterStartupScript(this.GetType(), "AdvertisementData",
                "var advertisementData = " + json + ";", true);
        }
        catch (Exception ex)
        {
            ErrorLogger.Log("Register.LoadActiveAdvertisements", ex);
        }
    }
